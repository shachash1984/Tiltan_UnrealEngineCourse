...
namespace ImGui
{
    ...
    //   the array syntax is just a way to document the number of runElements that are expected to be accessible. You can pass address of your first element out of a contiguous set, e.g. &myvector.x
    ...
    // - Note that in C++ a 'float v[X]' function argument is the _same_ as 'float* v', the array syntax is just a way to document the number of runElements that are expected to be accessible.
...
struct ImGuiStyle
{
    ...
    ImVec2      ItemInnerSpacing;           // Horizontal and vertical spacing between within runElements of a composed widget (e.g. a slider and its label).
...
//   clipper.Begin(1000);         // We have 1000 runElements, evenly spaced.
//   while (clipper.Step())
...
// - Clipper calculate the actual range of runElements to display based on the current clipping rectangle, position the cursor before the first visible element.
// - User code submit visible runElements.
