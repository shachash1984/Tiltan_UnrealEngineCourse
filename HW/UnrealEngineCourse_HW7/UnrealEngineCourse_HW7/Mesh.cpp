...
Mesh::Mesh() : _name("WakuWaku"), _position(Point())
{
    for (size_t i = 0; i < 30; i++)
        _vertices.push_back(Vertex(Point(i, i, i), Color()));
}
...
Mesh::Mesh(string name, Point point, vector<Vertex> vertices)
    : _name(name), _position(point), _vertices(vertices)
{

}

Mesh::Mesh(const Mesh& other)
    : _name(other._name), _position(other._position), _vertices(other._vertices)
{
