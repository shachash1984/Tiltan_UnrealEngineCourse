...
public:
	std::vector<std::string> parse(std::string filename) override {
		...
		if (!file.is_open()) {
			throw std::runtime_error("Failed to open file: " + filename); //-10 no exception handling - we havent learned it 
		}
		std::string line;
...
public:
	...
	void run() {
		try {
			...
			//SetWindowPos(xPos_, yPos_); // use position ***-10 This does not compile***
			...
			if (nextDialogId == 0) {
				std::cout << "Dialog complete./Sorry, that part of the game isn't finished yet." << std::endl;
				return;
			}
